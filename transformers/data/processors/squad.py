...
def squad_convert_examples_to_features(examples, tokenizer, max_seq_length,
                                       ...
                                       return_dataset=False, is_debug=0):
    """
    ...
    for (example_index, example) in enumerate(tqdm(examples)):
        if is_debug and example_index>10:
            break
        ...
        while len(spans) * doc_stride < len(all_doc_tokens):
            ...
            input_ids_len = sum([int(id != 0) for id in encoded_dict["input_ids"]])
            sep_tok_len=1
            special_tok_len=3
            t_c_len=len(truncated_query)
            t_q_len=input_ids_len-special_tok_len-t_c_len

            assert(encoded_dict["input_ids"][1+t_c_len]==tokenizer.sep_token_id)

            t_pq_end_pos = [1 + t_c_len - 1, 1 + t_c_len + sep_tok_len + t_q_len - 1]
            ...
            encoded_dict["pq_end_pos"] = t_pq_end_pos
    ...
    if return_dataset == 'pt':
        ...
        if not is_training:
            ...
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                    all_example_index, all_cls_index, all_p_mask, all_pq_end_pos)
        else:
            all_start_positions = torch.tensor([f.start_position for f in features], dtype=torch.long)
            all_end_positions = torch.tensor([f.end_position for f in features], dtype=torch.long)
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                    all_start_positions, all_end_positions,
                                    all_cls_index, all_p_mask, all_pq_end_pos)
...
class SquadProcessor(DataProcessor):
    ...
    def get_examples_from_dataset(self, dataset, evaluate=False):
        ...
        examples = []
        for i,tensor_dict in enumerate(tqdm(dataset)):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        examples = []
        for k,entry in enumerate(tqdm(input_data)):
            # if k>5:
            #     break
            title = entry['title']
